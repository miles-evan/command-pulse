...
export async function createAndAssignShift(shiftStart, shiftEnd, location, payRate, userId, shiftRequestMessage, companyId, assignerUserId) {
...
export async function getAllShifts(companyId, startDate, endDate) {
    ...
    (await Promise.all(users.map(user => Shift.find({
        _id: { $in: user.shiftIds },
        shiftStart: { $gte: startDate, $lt: endDate },
