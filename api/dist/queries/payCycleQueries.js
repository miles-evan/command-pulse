...
async function computeSummary(shifts, payCycle) {
    ...
    const existingShiftIds = new Set((await Shift.find({ _id: { $in: shifts.map(s => s.shiftId) } }, "_id")).map(shift => shift._id.toString()));
    // compute summary
    const summary = { totalHoursWorked: 0, totalHoursWorkedRevised: 0, totalEarning: 0, totalEarningRevised: 0 };
    for (const shift of shifts) {
        shift.hoursWorked = shift.clockInTime ? (shift.shiftEnd - shift.shiftStart) / (1000 * 60 * 60) : 0;
    ...
    // clean payCycle
    await PayCycle.findByIdAndUpdate(payCycle?.payCycleId, { $pull: {
...
export async function confirmPaymentSent(userId, startDate, endDate, payCycleId = null, paymentMethod = "cash") {
    if (!payCycleId)
        payCycleId = await createPayCycle(userId, startDate, endDate);
