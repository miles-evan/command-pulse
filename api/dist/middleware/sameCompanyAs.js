...
export function sameCompanyAsUser(pathFromRequestToUserId) {
    ...
    return async (request, response, next) => {
        ...
        if (!userId)
            return next();
        if (await usersInSameCompany(request.user.id, userId))
            return next();
...
export function sameCompanyAsShiftRequest(pathFromRequestToShiftRequestId) {
    ...
    return async (request, response, next) => {
        ...
        if (await userInSameCompanyAsShiftRequest(request.user.id, shiftRequestId))
            return next();
        return response.status(403).send({ message: "must be in the same company as shift" });
    };
}
export function sameCompanyAsIncidentReport(pathFromRequestToIncidentReportId) {
    ...
    return async (request, response, next) => {
        ...
        if (await userInSameCompanyAsIncidentReport(request.user.id, incidentReportId))
            return next();
